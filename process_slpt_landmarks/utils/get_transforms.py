...
def get_warp_mat(cur_cxy, cur_len, det_size):
    """
    Keep the center of rect unchange, 'resize' the crop box to det_size without deformation;
    """
    mean_len = det_size / 2.
    mean_cxy = np.array([det_size / 2., det_size / 2.], dtype=np.float32)
    warp_scale = mean_len / cur_len
    warp_trans = mean_cxy - cur_cxy * warp_scale
    warp_mat = np.array((warp_scale, 0, warp_trans[0], 0, warp_scale, warp_trans[1]), dtype=np.float32).reshape(2, 3)
    
    return warp_scale, warp_trans, warp_mat    
